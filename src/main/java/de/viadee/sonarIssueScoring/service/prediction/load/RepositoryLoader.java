...
public class RepositoryLoader {
    ...
    public SplitRepository loadSplitRepository(PredictionParams params) {
        try (Git git = repositoryCache.getRepository(params.gitServer())) {
            ...
            int snapshotOffset = params.predictionHorizon() / 2;
            for (int offset = 0; offset + params.predictionHorizon() < history.size() && offset < snapshotOffset * 10; offset += snapshotOffset)
