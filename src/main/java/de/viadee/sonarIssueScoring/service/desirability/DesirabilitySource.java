...
public class DesirabilitySource {
    ...
    public Map<String, IssueDesirability> calculateIssueDesirability(UserPreferences userPreferences) {
        ...
        PathSuffixLookup<Path> realPathLookup = new PathSuffixLookup<>(Maps.uniqueIndex(prediction.results().keySet(), p -> p));
    ...
    private IssueDesirability createDesirabilityInformation(Issue issue, PathSuffixLookup<Path> realPathLookup, PredictionResult prediction,
        ...
        Path path = Paths.get(issue.getComponent().substring(issue.getComponent().lastIndexOf(':') + 1));
